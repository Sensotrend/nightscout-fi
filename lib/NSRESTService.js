...
function NSRestServer (env) {
   ...
   NightscoutRESTServer.getAsync('/entries', getUser, async (req, res) => {
      ...
      let entries = await Entries.getEntries('https://fhirsandbox2.kanta.fi/phr-resourceserver/baseStu3', req.user.sub, token);
      res.send(entries);
   });
   ...
   NightscoutRESTServer.postAsync('/entries', getUser, async (req, res) => {
      ...
      let token = await env.userProvider.getAccessTokenForUser(req.user);
      let entries = await Entries.postEntries('https://fhirsandbox2.kanta.fi/phr-resourceserver/baseStu3', req.user.sub, token, req.body);
      res.send(entries);
   });
