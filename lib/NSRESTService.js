...
function NSRestServer (env) {
   ...
   NightscoutRESTServer.getAsync('/treatments*', loadUserIntoSessionOrFail, async (req, res) => {
      env.requestLogger.startLogging(req.user.user_id, req);
      ...
      env.requestLogger.addSensitiveLogValue('access_key', req.user.site_secret);
      env.requestLogger.addSensitiveLogValue('entries_returned', treatments);
   ...
   async function putOrPostTreatment (req, res) {
      ...
      let data = req.body;
      if (!Array.isArray(data) && (data.created_at || data.date)) data = [data];

      env.requestLogger.startLogging(req.user.user_id, req);
      env.requestLogger.addSensitiveLogValue('nightscout_sourcedata', data);

      if (Array.isArray(data)) {
         env.requestLogger.setLogValue('http_status', 200);
         const token = await env.oauthProvider.getAccessTokenForUser(req.user);
         const { uploadResults, records } = await Treatments.postTreatments(env.FHIRServer, req.user.sub, token, data);

         env.requestLogger.addSensitiveLogValue('access_key', req.user.site_secret);
         env.requestLogger.setLogValue('fhir_operation', 'create');
         env.requestLogger.addLogData(uploadResults);
         env.requestLogger.addSensitiveLogValue('fhir_data', records);

         res.send(true);

      } else {

         logger.error(`${req.method} body missing or data not json`);
         res.status(400).send(`${req.method} body missing or data not json`);
         env.requestLogger.setLogValue('http_status', 400);
   ...
   NightscoutRESTServer.getAsync('/entries*', loadUserIntoSessionOrFail, async (req, res) => {
      ...
      env.requestLogger.addSensitiveLogValue('entries_returned', entries);
      env.requestLogger.setLogValue('http_status', 200);
      env.requestLogger.writeLog();
      res.send(entries);
   });
   ...
   NightscoutRESTServer.postAsync('/entries*', loadUserIntoSessionOrFail, async (req, res) => {
      ...
      const token = await env.oauthProvider.getAccessTokenForUser(req.user);
      env.requestLogger.startLogging(req.user.user_id, req);
      env.requestLogger.addSensitiveLogValue('nightscout_sourcedata', req.body);
      env.requestLogger.addSensitiveLogValue('access_key', req.user.site_secret);
   ...
   NightscoutRESTServer.getAsync('/devicestatus*', loadUserIntoSessionOrFail, async (req, res) => {
      env.requestLogger.startLogging(req.user.user_id, req);
      env.requestLogger.addSensitiveLogValue('status_returned', []);
      env.requestLogger.writeLog();
      res.send([]);
   });
   ...
   NightscoutRESTServer.getAsync('/status*', loadUserIntoSessionOrFail, async (req, res) => {
      ...
      env.requestLogger.setLogValue('statusresponse', response);
      env.requestLogger.writeLog();
