...
function FIPHR (env) {
   ...
   const client = new fiphrIssuer.Client(process.env.NODE_ENV === 'production' ? productionOpts : sandboxOpts); // => Client
   ...
   app.get('/callback', async (req, res) => {
      ...
      try {
         ...
         try {
            token = await client.oauthCallback(OauthCallbackURL, req.query, { state: req.session.state, response_type: 'code' }); // => Promise
         } catch (error) {
         ...
         env.logger.error('Access Token Error ' + error);
