...
//         const idTokenDecoded = jwt.decode(token.id_token);
 //        console.log("HCP ID", idTokenDecoded.fhirUser);
   ...
   app.get('/permissionRequestForm', async (req, res) => {
      generatePermissionRequestForm(req, res);
   });
   ...
   async function generatePermissionRequestForm (req, res) {
      ...
      console.log('Reading patient', req.session.token.patient);
      ...
      try {
         patientRequest = await fhirClient.read({
            resourceType: 'Patient',
            id: req.session.token.patient
         });
      } catch (error) {
         console.log('Error reading patient data', error);
      }
