  ...
  function fetchAccountData(account) {
    ...
    .then((sent) => {
      if (sent.length) {
        ...
        const latestEntryDate = sent.reduce((acc,cur) => {
		
          if( acc === undefined){
            acc = cur;
          }
          
          if( (new Date(cur.effectiveDateTime)).getTime() >= (new Date(acc.effectiveDateTime)).getTime() ){
            acc = cur;
          }
        ...
        account.latestEntryDate = latestEntryDate;
        accountLogger.log(
...
async function sendPersonalDataWarehouse(userInformation, entries){
  ...
  const user = await env.userProvider.findUserById(userInformation.userId);
  if (!user) {
  ...
  await NSEntries.postEntries(env.FHIRServer, userInformation.userId, token, entries);

  await CarelinkProcess.findOneAndUpdate({user_id: userInformation.userId},{error_date: null,
    ready_date: Date.now(), run_date: null
  }); 
...
async function createCareLinkConnect(userInformation){
  ...
  try{
    ...
    }else if( error.message === 'input.forEach is not a function'){
      logger.log(`Convert the entries is failed and error ${error}`);
      await CarelinkProcess.findOneAndUpdate({user_id: userInformation.userId},{error_date: Date.now(), run_date: null,  $push: { error_message: error.toString()}}, { new: true });
    }
    else{
...
export default async function carelinkProcessRun(){
      ...
      const findedProcessUsers = await CarelinkProcess.find({
        $and: [{run_date: {$eq: null}}],
        $or: [ {error_date: {$ne: null }},{ready_date: {$eq:null}},{ready_date: { $lte: new Date(
          Date.now() - (5*60*1000))}}]
      });
