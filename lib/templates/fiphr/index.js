...
function FIPHRDataProcessor () {
   ...
   function enrichFHIRObject (sourceData) {
      ...
      if (sourceData.text && sourceData.text.div) {
         const split = sourceData.text.div.split('<br />');

         split.forEach(function (e) {
            const keyValue = e.split(': ');
            console.log('Field:',keyValue[0]);
            switch (keyValue[0]) {
               case "Laite":
               case "Device":
                  entry.deviceId = keyValue[1].replace(' (via Sensotrend Connect)', '');
                  break;
               case "Muutos":
               console.log('Muutos:',keyValue[1]);
   ...
   function enrichObject (sourceData, patientReference) {
      ...
      //if (!entry.direction) { entry.direction = "Flat"; }
      //if (!entry.noise) { entry.noise = 1; }

      // Kudossokeri<br />
      // Aika: {{time_fhir}}<br />
      // Laite: {{deviceId}} (via Sensotrend Connect)<br />
      // Tulos: {{valueMmol}} mmol/l<br />
      // Muutos: {{deltaMmol}} mmol/l<br />
      // Suunta: {{direction}}<br />
      // Mittauslaatu: {{noise}}<br />

      textArray.push("Aika: " + entry.time_fhir);
      textArray.push("Laite: " + entry.deviceId + " (via Sensotrend Connect)");
