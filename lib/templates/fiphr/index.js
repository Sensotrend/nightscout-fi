...
function FIPHRDataProcessor (env) {
   ...
   async function loadTemplate (objectType) {
      ...
      try {
         ...
         logger.error('FHIR conversion error: template for object type "' + objectType + '" not found');
         return;
      }
   ...
   async function _convertRecordFromFHIR (sourceData, context) {
      ...
      try {
         ...
         if (!template) {
            logger.error('ALERT! Record type ' + sourceData.type + ' not handled');
            return;
         }
         ...
         logger.info('Hit an issue converting data on context: ' + context + ' ' + JSON.stringify(sourceData, null, 2) + ' ' + error);
      }
   }
   ...
   async function convertRecord (sourceData, patientReference) {
      ...
      if (!sourceData.type || !sourceData.time || !sourceData.deviceId) {
         logger.error('ALERT! Record type, time or device missing, cannot convert data');
         return;
      }
   ...
   FIPHRDataProcessor.exportRecords = async function (input, options) {
      ...
      if (!options.FHIR_userid) {
         logger.info('options.FHIR_userid needed for FHIR exporting');
         return false;
      }
