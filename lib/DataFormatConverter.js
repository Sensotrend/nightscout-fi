...
import { getTemplate } from './FHIRTemplates';

const UUID_NAMESPACE = 'd040ecfe-2dd1-11e9-9178-f7b0f1a319bd';
const uuidv5 = require('uuid/v5');

export const convertToFHIR = async function (sourceData, patientReference) {
...
async function convertRecord (sourceData, patientReference) {
   ...
   var template = await getTemplate(sourceData.type);
   if (!template) {
...
export const convertRecordFromFHIR = async function (sourceData, context) {
...
async function _convertRecordFromFHIR (sourceData, context) {
   ...
   try {
      ...
      var template = await getTemplate(type + code, context);
