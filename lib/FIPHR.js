...
function FIPHR (authmanager) {
   ...
   console.log('using stragety:', strategy);

   // Strategy config
   FIPHR.FIPHRStrategy = new OauthStrategy(strategy
   ...
   FIPHR.findUserBySiteSecret = async function (siteSecret) {
      return await _auth.findUserBySiteSecret(siteSecret);
   }

   FIPHR.findUserById = async function (id) {
   ...
   FIPHR.findUserBySiteId = async function (siteID, sitePW) {
