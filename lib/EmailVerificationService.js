...
const EMAILVERIFICATION_BASE_URL = process.env.EMAILVERIFICATION_BASE_URL || 'https://connect.sensotrend.fi/emailverification';
...
function EmailVerificationService (env) {
   ...
   app.post('/sendverificationrequest', async (req, res) => {
      ...
      if (req.body.password) {
         options.password = Auth.sha1(req.body.password);
      } else {
         const user = await env.userProvider.findUserById(req.session.user.user_id);
         if (!user || !user.password) {
            console.log('Unable to load password', req.session.user.user_id, user);
      ...
      const msg = {
         ...
         templateId: 'd-5c15d3f09ea549e98146ae8b9057c542',
   ...
   app.get('/verifyemail/:requestid', async (req, res) => {
      ...
      user.password = Auth.sha1(emailRequest.password);
      ...
      try {
         ...
         const msg = {
            to: user.email,
            from: 'info@sensotrend.com',
            templateId: 'd-1191318adc2a465a9483ffdee13eefaa',
            dynamic_template_data: {},
         };
