...
function EmailVerificationService (env) {
   ...
   app.get('/verifyemail/:requestid', async (req, res) => {
      ...
      if (permission) {
         console.log('Verifying verification request', requestid);
         return res.redirect(`${frontend}/?verified`);
      }
      console.warn('Rejecting email verification request', requestid);
      // return res.status(400).json({ status: 'Invalid request ID.' });
