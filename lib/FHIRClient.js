...
function FHIRClient (URL, { patient, bearertoken, env } = {}) {
   ...
   if (env && env.https_certificate) {
      options.requestOptions.cert = env.https_certificate;
      options.requestOptions.key = env.https_privateKey;
   }

   if (env && env.http_basic_auth) {
      options.customHeaders = {
         'Authorization': 'Basic ' + env.http_basic_auth 
       }
   }
