...
function FHIRClient (URL, { patient, bearertoken, env } = {}) {
   ...
   FHIRClient.uploadBatch = async function (records) {
      ...
      for (let i = 0; i < records.length / 100; i += 1) {
         const smallerBatch = records.slice(i * 100, 1 * 100 + 100);
         const bundle = {
            resourceType: 'Bundle',
            type: 'batch',
            entry: smallerBatch.map(r => ({
               resource: r,
               request: {
                  url: 'Observation/',
                  method: 'POST',
         ...
         .then((data) => {
            logger.debug('Received data from bundle upload:\n' + JSON.stringify(data));
            // TODO: process results... See which ones were skipped
            uploadResults.created += smallerBatch.length || 1;
         })
         .catch((error) => {
            logger.error('Problem with batch upload:\n' + JSON.stringify(error));
            uploadResults.errors += smallerBatch.length || 1;
            uploadResults.error = error.message;
            logger.error('FHIR client: error creating object', JSON.stringify(error));
         });
      }
