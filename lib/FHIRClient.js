...
function FHIRClient(URL, { patient, bearertoken, env } = {}) {
   ...
   if (env && env.https_certificate) {
      options.https.certificate = env.https_certificate;
      options.https.key = env.https_privateKey;
   }
   ...
   FHIRClient.getResultsOfType = async function (type, patientId) {
      logger.info('FHIRClient for patient ' + patient + ' fetching ' + type + ' records for patientId ' + patientId + ' using search parameters ' + JSON.stringify(searchparams));
   ...
   FHIRClient.search = async function (type, searchparams) {
      logger.info('FHIRClient for patient ' + patient + ' searching ' + type + ' records for patientId ' + patientId + ' using search parameters ' + JSON.stringify(searchparams));
      const results = await _fhirClient.search({
         resourceType: type,
