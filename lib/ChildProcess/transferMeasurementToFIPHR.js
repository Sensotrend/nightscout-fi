...
async function findDatasetAndSplitSmallParts(){
    return new Promise(async (resolve ,reject) => {
        ...
        const tempStorage = await DatesetTempStorage.find({_id: {$in: tempStorageLock}});
        
        await session.commitTransaction();
        session.endSession();

        try{
                let processIsOn = await DatesetTempStorage.find({ pid:processId, status: 'process' });
...
async function convertAndSave(){
 ...
 async function initilization(){
        ...
        const findProcessExists =  await DatesetTempStorage.find({ process_date: {$ne: null}});
        ...
        if( processNotExists.length > 0 ){
          const updateDataSetProcessPidToNull = await DatesetTempStorage.updateMany({ process_date: {$ne: null},pid: { $in: processNotExists }}, { pid: null });
          logger.info(`Update dataset process pid to null: ${updateDataSetProcessPidToNull}`);
        }
        
        const howManyProcess = await DatesetTempStorage.find({ process_date: {$ne: null}, pid: { $eq: null} });
        ...
        howManyProcess.forEach( async (element) => {
            await DatesetTempStorage.updateOne({_id: element._id},
                { upload_date: null, process_date: null, create_date: new Date(),
                    status: 'not_sended',initilization_date: new Date(), pid: processId },{ new: true });
        })
