...
const HOW_MANY_INCLUDED_IN_PROSESSING = 1;
...
async function convertAndSaveToFhir (element) {
    ...
    return new Promise(async ( resolve, reject ) => {
        ...
        await DatesetTempStorage.updateOne({_id: element._id},{pid:processId, process_date: new Date(), status: 'process' },{ new: true });
...
async function findDatasetAndSplitSmallParts(){
    return new Promise(async (resolve ,reject) => {
            try{
                    ...
                    const session = await env.mongo.getConnection().startSession();
                    session.startTransaction();

                    const filter = {pid: {$eq: null}, $or: [{status: 'not_sended'},{status: 'error'}] };
                    ...
                    logger.info(`${processId} and data from database: ${tempStorage}`);
                ...
                logger.info(`Mongoose write error: ${error}`);
                reject('Mongoose write error ' + error);
            }    
    })
 }
...
async function deleteAllFinishedProcessed(){
    return new Promise(async (resolve ,reject) => {
        try{
            ...
            const deleteResult = await DatesetTempStorage.deleteMany({ pid: processId, $or: [{status: 'ok'},{how_many_times_try: 0},{upload_date: { $ne: null}}]}) 
...
async function convertAndSave(datasets){
 ...
 async function initilization(){
    ...
    try{
        ...
        const findProcessExists =  await DatesetTempStorage.find({ process_date: {$ne: null}}).session(session);
        ...
        if( processNotExists.length > 0 ){
          ...
          logger.info(`Update dataset process pid to null: ${updateDataSetProcessPidToNull}`);
        }
        
        const howManyProcess = await DatesetTempStorage.find({ process_date: {$ne: null}, pid: { $eq: null} }).session(session);
        
        logger.info(howManyProcess);
        howManyProcess.forEach( async (element) => {
            await DatesetTempStorage.updateOne({_id: element._id},
                { upload_date: null, process_date: null, create_date: new Date(),
                    status: 'not_sended', pid: processId },{ session: session, new: true });
        })
        ...
        resolve('Initilization complite')

    }catch(error){
        logger.error('Error when two instance try changes values same time')
        reject(`Error when two instance try changes values same time ${error}`)
    }
    ...
    return new Promise((resolve, reject) => {
        try{
          let timerId = setTimeout( async () => {
             ...
             let datasets =  await findDatasetAndSplitSmallParts();
 ...
 async function run(){
    ...
    while(true){
        try{
            ...
            logger.error(`Error when runChildProcess ${error}`);
        }
    }
 }
