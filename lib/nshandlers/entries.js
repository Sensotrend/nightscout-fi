...
function NSEntries (env) {
   ...
   NSEntries.getEntries = async function (fhirserver, userid, token, query) {
      ...
      console.log('Got Glucometer entries, count ' + records.total);
      ...
      console.log('Got CGM entries, count ' + records2.total);
   ...
   NSEntries.postEntries = async function (fhirserver, userid, token, entries) {
      ...
      var FHIRClient = new _FHIRClient(fhirserver, userid, token, env);
