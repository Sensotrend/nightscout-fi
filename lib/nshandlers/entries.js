...
import { convertRecordFromFHIR, convertToFHIR } from '../DataFormatConverter';
...
function NSEntries () {
   ...
   NSEntries.getEntries = async function (fhirserver, userid, token) {
      ...
      if (allRecords.length > 0) {
         nsRecords = await convertRecordFromFHIR(allRecords, 'nightscout');
      }
...
[
   ...
   NSEntries.postEntries = async function (fhirserver, userid, token, entries) {
      ...
      var records = await convertToFHIR(r, userid);
