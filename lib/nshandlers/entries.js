...
function NSEntries (env) {
   ...
   const requestLogger = env.requestLogger;
   ...
   NSEntries.getEntries = async function (fhirserver, userid, token, query) {
      ...
      env.requestLogger.setLogValue('fhir_data', allRecords);
      ...
      if (r.length > finalCount) r = r.splice(finalCount, r.length);
   ...
   NSEntries.postEntries = async function (fhirserver, userid, token, entries) {
      ...
      return { success: operationOutcome, uploadResults, records };
   };
