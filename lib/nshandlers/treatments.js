...
function NSTreatments (env) {
   ...
   NSTreatments.getTreatments = async function (fhirserver, userid, token, query) {
      ...
      console.log('Got Carb entries, count ' + records.total);
      ...
      console.log('Got MedicationAdministration records, count ' + records2.total);
      ...
      records2.entry.forEach(function (entry) {
         console.log('Got codes, ', getMedicationCodes(entry.resource));
      });  

     const filtered = records2.entry.filter(function(record) {
