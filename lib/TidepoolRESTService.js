...
export const batchSupported = true; // TODO: check from capability statement! Or at least from ENV.
const chunkSize = 500;
   ...
   uploadApp.postAsync('/data/:userId', async function (req, res) {
      ...
      if (SKIP_OLD_RECORDS) {
         ...
         if (latestDeviceDates) {
            logger.info('Adding skip dates ' + latestDeviceDates);
   ...
   dataApp.postAsync('/v1/datasets/:datasetId/data', async function (req, res) {
      ...
      if (dataSet) {
         ...
         for (let i = 0; i < records.length / chunkSize; i += 1) {
            ...
            if (chunk.length > 1 && batchSupported) {
               promises.push(
                  FHIRClient.uploadBatch(chunk)
                  .then((chunkResults) => {
                     (chunkResults.records || ['?']).forEach((r) => {
                        res.write(((uploadResults.records.length) > 1) ? ',' : '')
                        + JSON.stringify(r);
                        logger.info('***' + ((uploadResults.records.length) > 1) ? ',' : '')
                        + JSON.stringify(r);
                     ...
                     logger.info('Sent a batch chunk, results: ' + JSON.stringify(chunkResults));
                  })
                  .catch((error) => {
                     logger.warn('Failed sending batch chunk ' + JSON.stringify(chunk) + ':\n'
               ...
               for (const record of records) {
                  promises.push(() => {
                     ...
                     .then(() => {
                        if (singleUploadResults.errors.length === 0) {
                           res.write(((uploadResults.records.length) > 1) ? ',' : '')
                              + JSON.stringify(singleUploadResults.records[0]);
                           logger.info('***' + ((uploadResults.records.length) > 1) ? ',' : '')
                              + JSON.stringify(singleUploadResults.records[0]);
                           uploadResults.records.push(singleUploadResults.records[0]);
                           logger.info('Sent record ' + recordId + ', results: ' + JSON.stringify(uploadResults));
                        } else {
                           logger.warn('Failed sending record ' + recordId + ', results: ' + JSON.stringify(uploadResults));
                        }
                     })
                     .catch((error) => {
                        logger.warn('Failed sending record ' + JSON.stringify(record) + ':\n' + JSON.stringify(error));
                     })
                  });
               }
            }
         ...
         .then(() => {
            ...
            logger.info('],');
            logger.info(`"created": ${uploadResults.created},`);
            logger.info(`"skipped": ${uploadResults.skipped},`);
            logger.info(`"errors": ${uploadResults.errors},`);
            logger.info(`"success": ${JSON.stringify(uploadResults.errors.length === 0) ? 1 : 0}}`);
