...
async function saveSetTempStorage(dataset, reqData){
   const reqBody = reqData.map((value) => value);
 
   const dataSetTempStorage = new DatesetTempStorage({
      user_id: dataset.user_id,
      dataset_id: dataset.dataset_id,
      deviceInformation: BSON.serialize(reqBody)
   });
   ...
   dataApp.postAsync('/v1/datasets/:datasetId/data', async function (req, res) {
     ...
     // logger.info(JSON.stringify(req.body));
      ...
      if (dataSet) {
         if( process.env.USE_DATASET_TEMP_STORAGE === 'TRUE'){
            await saveSetTempStorage(dataSet,req.body);
