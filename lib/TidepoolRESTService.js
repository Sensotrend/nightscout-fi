...
function TidepoolRESTService (runtimeEnv) {
   ...
   uploadApp.postAsync('/data/:userId', async function (req, res, next) {
      ...
      var FHIRClient = new(require('./lib/FHIRClient'))(req.userInfo.server, req.userInfo.userid);
      var patientRef = req.userInfo.userid;
   ...
   app.postAsync('/auth/login', async function (req, res, next) {
      ...
      if (u) {
         response = {
            data: {
               "name": "John Doe"
               , "userid": '5ba3d7aa-35bd-11e9-bb72-8f8be337adf1'
               , 'server': 'http://fhirsandbox.kanta.fi/phr-resourceserver/baseStu3'
            }
   ...
   */
   ...
   app.get('/access/groups/:groupid', (req, res) => {
      //    console.log("BODY: " + JSON.stringify(req.body));
      //    console.log("PARAMS: " + JSON.stringify(req.params));
      //    console.log("HEADERS: " + JSON.stringify(req.headers));
      console.log('Giving list of PWDs this account manages: /access/groups/:groupid ' + req.params.groupid);
      console.log(groups);
