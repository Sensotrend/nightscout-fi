...
function TidepoolRESTService (env) {
   ...
   const DataFormatConverter = _DataConverter(env);
   ...
   const worker = env.monq.worker(['datasetUploads']);
   ...
   async function cleanOldMonQJobs (env) {

      try {
        const db = env.mongo.getConnection();
        const myquery = { status: "complete" };
        const results = await db.collection("jobs").deleteMany(myquery);
        logger.info(results.result.n + " old job(s) deleted");
      } catch (err) {
   ...
   const job = new cron.CronJob('*/30 * * * *', function () {
      logger.info('Cleaning completed MonQ upload jobs');
      cleanOldMonQJobs(env);
   }, null, true);

   logger.info('Cleaning old MonQ Jobs');
   cleanOldMonQJobs(env);
   ...
   dataApp.postAsync('/v1/datasets/:datasetId/data', async function (req, res) {
      ...
      if (dataSet) {
         ...
         const queue = env.monq.queue('datasetUploads');

         queue.enqueue('uploadToFhir', job, function (err, job) {
            logger.info('enqueued a job with id: ' + req.params.datasetId);
         });
         res.status(200).send({
            "success": 1
         });
