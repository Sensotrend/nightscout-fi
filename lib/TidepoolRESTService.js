...
function TidepoolRESTService (env) {
   ...
   async function uploadRecordsAndUpdateResponse(userId, data, res) {
      ...
      let latestDeviceDates = {};
      if (SKIP_OLD_RECORDS) {
         latestDeviceDates = await env.lastSeenService.getLatestDates(patientRef);
         if (latestDeviceDates) {
            logger.info('Latest uploads from devices, ' + JSON.stringify(latestDeviceDates));
            converterOptions.skipRecordsUsingDates = latestDeviceDates;
         } else {
            logger.info('No stored latest upload times.');
         }
      } else {
         logger.info('Not skipping old records.');
      }
         ...
         if (records.length > 1 && batchSupported) {
            ...
            for (const record of records) {
               ...
               try {
                  ...
                  latestDeviceDates = singleUploadResults.latestDates;
