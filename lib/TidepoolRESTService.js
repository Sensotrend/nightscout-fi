      ...
      if (dataSet) {
         ...
         for (let i = 0; i < records.length / chunkSize; i += 1) {
            ...
            if (chunk.length > 1 && batchSupported) {
               const chunkResults = await FHIRClient.uploadBatch(chunk);
               (chunkResults.records || ['?']).forEach((r, i) => {
                  if (i > 0) {
                     res.write(',');
                  }
               ...
               for (const record of records) {
                  ...
                  if (singleUploadResults.errors.length === 0) {
                     if ((uploadResults.records.length) > 1) {
                        res.write(',');
                     }
                     res.write(JSON.stringify(singleUploadResults.records[0]));
