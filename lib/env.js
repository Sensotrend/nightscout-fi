...
function Environment () {
   ...
   const TOKEN_ENCRYPTION_KEY = process.env.TOKEN_ENCRYPTION_KEY || '123456789a';
   if (!TOKEN_ENCRYPTION_KEY) {
      env.logger.error('TOKEN_ENCRYPTION_KEY missing, cannot start');
      process.exit();
   }
   const FHIR_SERVER = process.env.FHIR_SERVER || 'https://fhirsandbox2.kanta.fi/phr-resourceserver/baseStu3';
   if (!FHIR_SERVER) {
...
//   if (process.env.MONGODB_URI) {
      env.MONGODB_URI = process.env.MONGODB_URI || 'mongodb://localhost:27017/sensotrend_connect_db';
      env.mongo = Mongo(env);
//   }
